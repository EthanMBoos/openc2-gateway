...
type vehicleSeq struct {
	hwm     uint32 // High-water mark (last accepted seq)
...
//
// Deduplication rules:
//   - First message from a vehicle: always accept, set hwm = seq
//   - seq > hwm: accept, update hwm = seq
//   - seq <= hwm: reject (duplicate or out-of-order old message)
//   - Wrap-around: if seq is much smaller than hwm (diff > 2^31), assume wrap and accept
func (st *SequenceTracker) Accept(vehicleID string, seq uint32) bool {
	...
	if !exists {
		...
		st.vehicles[vehicleID] = &vehicleSeq{hwm: seq, seen: true}
		return true
	}
	...
	// Check for wrap-around: if seq is much smaller than hwm, it likely wrapped
	// We use 2^31 as the threshold - if the difference is larger, assume wrap
	if seqAfter(seq, v.hwm) {
		v.hwm = seq
		return true
	}

	// seq <= hwm (accounting for wrap) - duplicate or stale
	return false
}
...
// would otherwise be rejected as stale (seq < hwm). The status state machine detects
